...
class prankCall{
	...
	private function loop($many,$sleep=null){
		...
		while($a<$many){
			...
			if(empty($ekse['challengeID'])){
				...
				$nn = $a+1;
				echo "\n Совершенно $nn звонка(звонков)!\e[0m";
				$a++;
			}
			if($sleep!=null) sleep($sleep);
			if($a>=$many) echo " \e[91mСпам завершён!\e[0m\n";
		}
	}
	...
	public function run(){
		while(true){
			echo " \e[34mПодряд [y/n]: ";
			$loop = $this->get();
			if($loop=="y" OR $loop=="n"){
				break;
			}else{
				echo " \e[41mНадо написать маленькую Y !!!\n\e[0m";
				continue;
			}
		}
		if($loop=="y"){
			echo " \e[34mКол-во звонков: ";
			$many = $this->get();
			$this->loop($many);
		}else{
			$this->ekse();
...
echo " \e[41mCalll Graber\e[0m - \e[92mmodification by: Blueindustrial\n	\e[96mvk.com/your_id.test\n";
echo " \e[34mНомер жертвы начиная с 7: ";
$no = get();
$n = new prankCall($no);
$n->run();
